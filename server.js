...
mongoose.connect(
  "mongodb+srv://first:K8EhXP5QExVJly73@cluster0.rtcpk.mongodb.net/second?retryWrites=true&w=majority"
);
var fs = require("fs");
...
const BOOK_URL = "https://isbndb.com/book/";
app.use(cors());
...
app.use("/", (req, res, next) => {
  try {
    if (
      req.path == "/login" ||
      req.path == "/register" ||
      req.path == "/get-productbyid" ||
      req.path == "/update-credits" ||
      req.path == "/update-buyer" ||
      req.path == "/get-user" ||
      req.path == "/get-pastOrders" ||
      req.path == "/cancle-product" ||
      req.path == "/get-book-data"
    ) {
      next();
    } else {
...
app.post("/add-product", upload.any(), (req, res) => {
  try {
    if (
      ...
      new_product.in_time = req.body.in_time;
      new_product.out_time = req.body.out_time;
...
app.post("/get-book-data", (req, res) => {
  try {
    console.log(req.body);
    if (req.body.isbn) {
      fetch(`${BOOK_URL}${req.body.isbn}`).then((data) => {
        console.log(data);
      });
    } else {
      res.status(500).json({ error: "ERROR - NO ISBN" });
    }
  } catch (err) {
    console.log(err);
  }
...
/* Api to is_delivered status */
app.post("/delivered-status", (req, res) => {
  try {
    if (req.body && req.body.id) {
      product.findByIdAndUpdate(
        req.body.id,
...
app.post("/update-credits", async (req, res) => {
  try {
    if (req.body) {
      let myUser = await user.findByIdAndUpdate(req.body.id, {
        $inc: { credits: req.body.value },
      });
...
app.post("/update-buyer", async (req, res) => {
  try {
    if (req.body) {
      ...
      let myUser = await product.findByIdAndUpdate(req.body.id, {
        consumer_id: req.body.value,
      });
      if (myUser) {
        res.status(200).json({
          status: true,
